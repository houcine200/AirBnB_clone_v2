...
from os import path
from fabric import local, put, run
...
def do_deploy(archive_path):
    try:
        ...
        archive_filename = archive_filename = archive_path.split("/")[-1].split(".")[0]
        release_path = f"/data/web_static/releases/{archive_filename}"
        run(f"mkdir -p {release_path}")
        run(f"tar -xzf /tmp/{archive_filename}.tgz -C {release_path} --strip-components=1")
